...
# ── GROQ AI ──────────────────────────────────────
def ask_groq(prompt, system="Siz matematik va umumiy bilim bo'yicha yordamchisiz. O'zbek tilida qisqa va aniq javob bering."):
    key = GROQ_KEY
...
def checkAiLimitBot(cid):
    ...
    today = get_today()
    # Kun yangilangan bo'lsa — hisobni reset qilish
    if u.get("aiDate") != today:
        u["aiUsed"] = 0
        u["aiDate"] = today
        users[uid] = u
        set_users(users)
    ...
    if used >= 20:
        send(cid,
            "❌ Bugungi AI limitingiz tugadi! (20/20)\n\n"
            "⏰ Limit ertaga yangilanadi\n\n"
            "Cheksiz foydalanish uchun Pirami kod kiriting:",
...
def useAiCountBot(cid):
    ...
    today = get_today()
    if uid not in users:
        users[uid] = {"id": uid, "aiUsed": 0, "aiDate": today, "isAdmin": False}
    # Kun yangilangan bo'lsa reset
    if users[uid].get("aiDate") != today:
        users[uid]["aiUsed"] = 0
        users[uid]["aiDate"] = today
...
def get_bot_username():
    ...
    try:
        ...
        if uname:
            db_set("ms_bot_username", uname)
        return uname
    except Exception:
        return ""


def ask_groq_image(image_url, prompt="Ushbu rasmdagi matematik masalani o'zbek tilida yech. Bosqichma-bosqich tushuntir."):
    """Groq vision orqali rasmni tahlil qilish"""
    key = GROQ_KEY
    raw = db_get("ms_ai_key")
    if raw and len(str(raw)) > 10:
        key = str(raw)
    if not key:
        return None, "❌ Groq API kalit sozlanmagan!"
    try:
        r = requests.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": "Bearer " + key, "Content-Type": "application/json"},
        ...
        if not r.ok:
            return None, "❌ Groq vision xatosi: " + str(r.status_code)
        data = r.json()
        text = data.get("choices", [{}])[0].get("message", {}).get("content", "")
        return text, None
    except Exception as e:
...
def process(update):
    try:
        if "message" in update:
            ...
            # ── GURUH UCHUN RASM TAHLIL ──────────────────
            if is_group and "photo" in msg:
                bot_uname = get_bot_username()
                caption   = msg.get("caption", "") or ""
                # Faqat @bot_nomi yoki /rasm buyrug'i bilan
                if ("@" + bot_uname in caption) or caption.startswith("/rasm") or caption.startswith("/photo"):
                    photo   = msg["photo"][-1]  # Eng yaxshi sifat
                    file_id = photo["file_id"]
                    # Telegram fayl URL olish
                    try:
                        fr = requests.get(TG_URL + "/getFile", params={"file_id": file_id}, timeout=10).json()
                        file_path = fr.get("result", {}).get("file_path", "")
                        image_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{file_path}"
                        if not checkAiLimitBot(cid):
                            return
                        send(cid, "⏳ Rasm tahlil qilinmoqda...")
                        reply, err = ask_groq_image(image_url)
                        if err:
                            send(cid, err)
                        else:
                            useAiCountBot(cid)
                        ...
                        send(cid, "❌ Rasm yuklab bo'lmadi: " + str(e))
                return

            # ── SHAXSIY CHAT UCHUN RASM TAHLIL ──────────
            if not is_group and "photo" in msg:
                photo   = msg["photo"][-1]
                file_id = photo["file_id"]
                try:
                    fr = requests.get(TG_URL + "/getFile", params={"file_id": file_id}, timeout=10).json()
            ...
            if is_group:
                bot_uname = get_bot_username()
                mention   = "@" + bot_uname if bot_uname else ""
                is_ask    = text and (
                    text.startswith("/ask") or
                    text.startswith("/savol") or
                    (mention and mention in text)
                )
                if not is_ask:
                    return  # Guruhda faqat mention yoki /ask da javob beradi

                # Mention yoki buyruqni olib tashlash
                clean_text = text
                if text.startswith("/ask "):
                    clean_text = text[5:].strip()
                elif text.startswith("/savol "):
                    clean_text = text[7:].strip()
                elif mention and mention in text:
                    clean_text = text.replace(mention, "").strip()

                if not clean_text:
                    send(cid, "❓ Savol yozing: /ask <savol> yoki @" + bot_uname + " <savol>")
                    return

                if not checkAiLimitBot(cid):
                    return
                send(cid, "⏳ AI javob yozmoqda...")
                reply, err = ask_groq(clean_text)
                if err:
                    send(cid, err)
                else:
                    useAiCountBot(cid)
